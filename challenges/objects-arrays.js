...
const contactInfo = graduates.map(grad => {
  let firstName = grad.first_name;
  let email = grad.email;
...
graduates.forEach(grad =>
  grad.university.toLowerCase().includes("uni") ? uni.push(grad.university) : null
)
console.log(uni);
...
*/
const populationTotal = zooAnimals.reduce((accumulator, population) => accumulator += population.population, 0);
